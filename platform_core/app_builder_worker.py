...
from platform_core.models import Artifact, Conversation, MemoryEntry, Project, ProjectPolicy, Workflow, WorkflowRun, WorkflowStepRun
...
BUILDER_STATUSES = {"builder_queued", "builder_running", "builder_completed", "builder_failed", "builder_cancelled"}


class BuilderSandboxManus(PolicyManus):
    async def execute_tool(self, command):
        enforce_tool_boundary(command.function.name, sandbox_enabled=False)
...
async def _create_agent(policy: ToolPolicy, role: str, model_profile: str | None):
    ...
    if settings.sandbox_enabled:
        if settings.sandbox_backend != "daytona":
            raise RuntimeError("App Builder requires the configured isolated Daytona sandbox in production")
    ...
    return await BuilderSandboxManus.create(
        llm=llm,
        policy_broker=PolicyToolBroker(policy),
        approved_tools=set(),
    )
...
async def _record_artifact(session, run: WorkflowRun, project: Project, local_path: Path) -> str:
    ...
    if size > settings.artifact_max_bytes:
        raise RuntimeError(f"Generated application archive exceeds {settings.artifact_max_bytes} byte limit")
...
async def process_builder_run(run_id: str) -> None:
    ...
    try:
        async with SessionLocal() as session:
            ...
            if run is None or run.status in {"builder_completed", "builder_failed", "builder_cancelled"}:
                return
            workflow = await session.get(Workflow, run.workflow_id)
            project = await session.get(Project, run.project_id)
            policy_row = await session.scalar(select(ProjectPolicy).where(ProjectPolicy.project_id == run.project_id))
        ...
        for step_index in range(run.current_step, len(workflow.steps_json)):
            ...
            if agent is None:
                agent = await _create_agent(policy, role, model_profile)
            else:
            ...
            for attempt in range(1, max_attempts + 1):
                async with SessionLocal() as session:
                    ...
                    await add_workflow_event(
                        ...
                        {"step_index": step_index, "name": spec["name"], "attempt": attempt, "role": role, "model_profile": profile.config_name},
                    )
                    await session.commit()
                ...
                try:
                    ...
                    last_error = str(exc)
                    logger.exception("Builder step %s attempt %s failed", step_index, attempt)
                    completed = False
            ...
            if not completed:
                async with SessionLocal() as session:
                    run = await session.get(WorkflowRun, run_id)
                    step = await session.scalar(select(WorkflowStepRun).where(WorkflowStepRun.workflow_run_id == run_id, WorkflowStepRun.step_index == step_index))
                    step.status = "failed"
                    step.error = last_error[:10000]
                    step.completed_at = datetime.now(timezone.utc)
                    run.status = "builder_failed"
                    run.error = f"Builder step {step_index} failed after {max_attempts} attempts: {last_error}"[:10000]
                    run.completed_at = datetime.now(timezone.utc)
                    await add_workflow_event(session, run_id, "builder.failed", {"status": run.status, "step_index": step_index, "attempts": max_attempts, "error": last_error[:2000]})
                    await session.commit()
                return

            input_tokens = int(getattr(agent.llm, "total_input_tokens", 0))
            output_tokens = int(getattr(agent.llm, "total_completion_tokens", 0))
            async with SessionLocal() as session:
                run = await session.get(WorkflowRun, run_id)
                step = await session.scalar(select(WorkflowStepRun).where(WorkflowStepRun.workflow_run_id == run_id, WorkflowStepRun.step_index == step_index))
                ...
                await add_workflow_event(
                    ...
                    {"step_index": step_index, "next_step": run.current_step, "input_tokens": input_tokens, "output_tokens": output_tokens},
                )
                await session.commit()
        ...
        async with SessionLocal() as session:
            ...
            run.completed_at = datetime.now(timezone.utc)
            if settings.sandbox_enabled:
                archive = _sandbox_package(agent, project.id)
            else:
                archive = _package_local_workspace(project.id)
            try:
                download_url = await _record_artifact(session, run, project, archive)
            finally:
                archive.unlink(missing_ok=True)
            await add_workflow_event(session, run_id, "builder.artifact_created", {"status": run.status, "download_url": download_url, "filename": "application.zip"})
            await add_workflow_event(session, run_id, "builder.completed", {"status": run.status, "artifact": download_url})
            await add_audit_event(session, "builder.completed", actor_user_id=run.owner_id, project_id=project.id, metadata={"run_id": run.id, "artifact": download_url})
            await session.commit()
    except Exception as exc:
        ...
        async with SessionLocal() as session:
            ...
            if run is not None:
                ...
                run.completed_at = datetime.now(timezone.utc)
                await add_workflow_event(session, run_id, "builder.failed", {"status": run.status, "error": run.error[:2000]})
                await add_audit_event(session, "builder.failed", actor_user_id=run.owner_id, project_id=run.project_id, metadata={"run_id": run.id})
                await session.commit()
    finally:
...
async def recover_stale_builder_runs() -> None:
    ...
    async with SessionLocal() as session:
        ...
        for run in rows.all():
            ...
            await add_workflow_event(session, run.id, "builder.recovered", {"status": "builder_queued", "reason": "stale heartbeat"})
        await session.commit()
