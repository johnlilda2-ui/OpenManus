...
from platform_core.research_bridge import apply_website_research_context
...
async def create_app_builder_run(
    ...
    if payload.mode == "website":
        raw_steps = build_website_builder_steps(project.id, payload.requirements)
        raw_steps = apply_website_research_context(
            raw_steps,
            synthesis=payload.research_synthesis,
            references=payload.research_references,
        )
        generated_description = "Generated by OpenManus Website Builder"
        default_name = "AI Website Builder"
    else:
        raw_steps = build_app_builder_steps(project.id, payload.requirements)
        generated_description = "Generated by OpenManus App Builder"
        default_name = "AI App Builder"
    ...
    workflow = Workflow(
        ...
        description=payload.description or generated_description,
        steps_json=steps,
    )
    ...
    await add_workflow_event(
        ...
        "builder.queued",
        {
            ...
            "mode": payload.mode,
            "research_context": bool(payload.research_synthesis or payload.research_references),
        },
    )
    ...
    try:
        ...
        async with SessionLocal() as recovery_session:
            await add_workflow_event(
                ...
                {"status": run.status, "reason": str(exc)[:1000], "mode": payload.mode},
            )
