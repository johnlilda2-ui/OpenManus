...
from platform_core.app_builder_api import router as app_builder_router
...
app = FastAPI(
    ...
    version="0.5.0",
    description="Persistent control plane around the OpenManus agent engine.",
    lifespan=lifespan,
)
app.add_middleware(
    CORSMiddleware,
...
async def get_or_create_policy(session: AsyncSession, project_id: str):
    from platform_core.models import ProjectPolicy
...
async def list_projects(
    ...
    result = await session.scalars(
        select(Project)
        .where(or_(Project.owner_id == user.id, Project.tenant_id.in_(membership_projects)))
    ...
    async def stream() -> AsyncIterator[str]:
        ...
        while True:
            ...
            if current_task is None or (
                current_task.status in {"completed", "failed", "cancelled"} and not sent
            ):
                return
